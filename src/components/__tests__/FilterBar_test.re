...
let activeFilters = (wrapper) => wrapper
  |> filters
  |> Enzyme.find(".active");
...
describe("FilterBar", () =>
  Expect.(describe("when the active filter is 'All'", () => {
    ...
    test("renders the other two filters with the .inactive class", () => {
      open! Expect.Operators;
    ...
    test("calls its onChange handler when a filter is clicked", () => {
